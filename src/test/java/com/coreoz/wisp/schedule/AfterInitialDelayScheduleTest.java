...
public class AfterInitialDelayScheduleTest {
	...
	public void first_execution_should_depends_only_on_the_first_delay() {
		Schedule after1msDelay = Schedules.afterInitialDelay(null, Duration.millis(1));
	...
	public void should_not_rely_only_on_job_executions_count() {
		Schedule every5ms = Schedules.fixedDelaySchedule(Duration.millis(5));
	...
	public void check_that_scheduler_really_rely_on_initial_delay() throws InterruptedException {
		...
		scheduler.schedule("job", Utils.doNothing(), Schedules.afterInitialDelay(Schedules.fixedDelaySchedule(Duration.standardSeconds(1)), Duration.ZERO));
